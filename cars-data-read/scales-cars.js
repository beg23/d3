...
function draw(cars) {
    ...
    const xScale = d3.scaleLinear()
                     .domain([Math.min(...cars.map(d => d.disp)), Math.max(...cars.map(d => d.disp))])
                     .range([0, plotWidth]);

    // mpg scale
    const yScale = d3.scaleLinear()
                     .domain([Math.min(...cars.map(d => d.mpg)), Math.max(...cars.map(d => d.mpg))])
                     .range([plotHeight, 0]);

    // wt scale
    const sizeScale = d3.scaleSqrt()
                        .domain([Math.min(...cars.map(d => d.wt)), Math.max(...cars.map(d => d.wt))])
                        .range([1, 10]);
